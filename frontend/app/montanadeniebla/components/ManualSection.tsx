...
export function ManualSection({ config }: { config: ManualConfig }) {
  ...
  const eyebrow = config.eyebrow?.trim() || 'MANUAL'
  const titulo = config.titulo?.trim() || 'REGLAS DE CAMPO'
  const piePagina =
    config.pie_pagina?.trim() ||
    'LM Parches Tácticos · Montaña de Niebla VII · Represalias'
