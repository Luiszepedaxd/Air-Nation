...
export function AirnationSection({ config }: { config: AirnationConfig }) {
  const eyebrow = config.eyebrow?.trim() || 'PLATAFORMA OFICIAL'
  const titulo =
    config.titulo?.trim() || 'Cobertura y comunidad en AirNation'
  const descripcion =
    config.descripcion?.trim() ||
    'Sigue Montaña de Niebla VII en AirNation: registro, galería y toda la información del operativo en un solo lugar.'
  const ctaTexto = config.cta_texto?.trim() || 'EXPLORAR AIRNATION'
  const ctaHref = config.cta_link?.trim() || 'https://www.airnation.online'
