...
function revalidateAll() {
  revalidatePath('/montanadeniebla')
  revalidatePath('/admin/montanadeniebla')
}

export async function upsertMontanaDeNieblaBlock(
  ...
  if (!(MDN_SLUGS as readonly string[]).includes(slug)) {
...
export async function updateBlockConfig(
  ...
  return upsertMontanaDeNieblaBlock(
    slug as MontanaDeNieblaSlug,
    config as Record<string, unknown>
  )
}

export async function toggleMontanaDeNieblaBlock(
  id: string,
  activo: boolean
...
export async function toggleBlockActive(
  ...
  let rowId: string
  if (!existing?.id) {
    const ins = await upsertMontanaDeNieblaBlock(slug as MontanaDeNieblaSlug, {})
  ...
  const toggled = await toggleMontanaDeNieblaBlock(rowId, activo)
  if ('error' in toggled) return toggled
  return { ok: true, id: rowId }
}

export async function reorderMontanaDeNieblaBlock(
  id: string,
  direction: 'up' | 'down',
  ...
  const r1 = await db.from('montanadeniebla_blocks').update({ orden: b.orden }).eq('id', a.id)
  const r2 = await db.from('montanadeniebla_blocks').update({ orden: a.orden }).eq('id', b.id)
...
export async function reorderAllBlocks(
  ...
  const updates = orderedIds.map((id, idx) =>
    db.from('montanadeniebla_blocks').update({ orden: idx + 1 }).eq('id', id)
  )
...
/*
 * ---------------------------------------------------------------------------
 * NOTA: Ejecutar manualmente en Supabase SQL Editor (fundador).
 * Tabla montanadeniebla_blocks + seed de 11 bloques — ver script SQL aparte.
 * ---------------------------------------------------------------------------
 */
