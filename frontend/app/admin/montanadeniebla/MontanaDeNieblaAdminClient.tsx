...
import type { GaleriaImagen } from '@/app/montanadeniebla/lib/types'
...
const SECTIONS: SectionDef[] = [
  { slug: 'hero', label: 'Hero — Cabecera', descripcion: 'Imagen o video de fondo, textos, símbolos y CTAs.' },
  { slug: 'narrativa', label: 'Narrativa', descripcion: 'Bloques de continuidad narrativa (edición + texto).' },
  { slug: 'sede', label: 'Sede', descripcion: 'Galería, dirección y mapa.' },
  { slug: 'countdown', label: 'Countdown', descripcion: 'Fecha objetivo y eyebrow.' },
  { slug: 'facciones', label: 'Facciones', descripcion: 'Red Sun y Lux et Umbra con código de uniforme.' },
  { slug: 'puntos_victoria', label: 'Puntos de victoria', descripcion: 'Criterios suma/resta del sistema de puntos.' },
  { slug: 'inscripcion', label: 'Inscripción', descripcion: 'Ventanas de precio escalonadas.' },
  { slug: 'sponsors', label: 'Sponsors', descripcion: 'Logos en marquesina por tier.' },
  { slug: 'manual', label: 'Manual de campo', descripcion: 'Reglas y lo que incluye el registro.' },
  { slug: 'galeria', label: 'Galería', descripcion: 'Imágenes de ediciones pasadas.' },
...
function validateConfig(slug: MontanaDeNieblaSlug, cfg: Record<string, unknown>): string | null {
  ...
  if (slug === 'facciones') {
    const w = (row: unknown) =>
      row && typeof row === 'object' && !Array.isArray(row)
        ...
        : ''
    const wr = w(cfg.red_sun)
    const wu = w(cfg.lux_et_umbra)
    if (typeof wr === 'string' && wr.trim() && !/^https?:\/\//i.test(wr.trim())) {
      return 'WhatsApp Red Sun debe ser URL http(s) (ej. https://wa.me/...) o vacío.'
    }
    if (typeof wu === 'string' && wu.trim() && !/^https?:\/\//i.test(wu.trim())) {
      return 'WhatsApp Lux et Umbra debe ser URL http(s) o vacío.'
    }
  }
  return null
}
...
function getThumb(slug: MontanaDeNieblaSlug, cfg: Record<string, unknown>): string {
  if (slug === 'hero') {
  ...
  if (slug === 'facciones') {
    const r = cfg.red_sun as { imagen_url?: string } | undefined
    return r?.imagen_url ?? ''
  }
  return ''
}
...
function sectionFor(slug: MontanaDeNieblaSlug): SectionDef {
  const def = SECTIONS.find((s) => s.slug === slug)
  if (!def) throw new Error(`Section metadata missing: ${slug}`)
  return def
}

export function MontanaDeNieblaAdminClient({
  initialBlocks,
}: {
  initialBlocks: MDNRecord[]
}) {
  const [configs, setConfigs] = useState<Record<MontanaDeNieblaSlug, Record<string, unknown>>>(() => {
    const map = {} as Record<MontanaDeNieblaSlug, Record<string, unknown>>
  ...
  const [ids, setIds] = useState<Record<MontanaDeNieblaSlug, string | null>>(() => {
    const map = {} as Record<MontanaDeNieblaSlug, string | null>
    for (const def of SECTIONS) {
      const found = initialBlocks.find((b) => b.slug === def.slug)
  ...
  const [activos, setActivos] = useState<Record<MontanaDeNieblaSlug, boolean>>(() => {
    const map = {} as Record<MontanaDeNieblaSlug, boolean>
    for (const def of SECTIONS) {
      const found = initialBlocks.find((b) => b.slug === def.slug)
  ...
  const [orderedSlugs, setOrderedSlugs] = useState<MontanaDeNieblaSlug[]>(() => {
    const fromBd = initialBlocks.map((b) => b.slug)
    const missing = MDN_SLUGS.filter((s) => !fromBd.includes(s))
    return [...fromBd, ...missing]
  })

  const [toggling, setToggling] = useState<MontanaDeNieblaSlug | null>(null)
  const [reordering, setReordering] = useState(false)
  const [reorderError, setReorderError] = useState<string | null>(null)
  const [expandido, setExpandido] = useState<MontanaDeNieblaSlug | null>(null)
  const [saving, setSaving] = useState<MontanaDeNieblaSlug | null>(null)
  const [saved, setSaved] = useState<MontanaDeNieblaSlug | null>(null)
  const [error, setError] = useState<string | null>(null)

  function cfg(slug: MontanaDeNieblaSlug): Record<string, unknown> {
    return configs[slug] ?? {}
  }

  function patch(slug: MontanaDeNieblaSlug, partial: Record<string, unknown>) {
    setConfigs((prev) => ({
      ...prev,
  ...
  function setField(slug: MontanaDeNieblaSlug, key: string, value: unknown) {
    setConfigs((prev) => ({
      ...prev,
  ...
  function str(slug: MontanaDeNieblaSlug, key: string): string {
    const v = configs[slug]?.[key]
    return typeof v === 'string' ? v : ''
  }

  async function handleSave(slug: MontanaDeNieblaSlug) {
  ...
  async function handleToggle(slug: MontanaDeNieblaSlug) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const oldIndex = orderedSlugs.indexOf(active.id as MontanaDeNieblaSlug)
    const newIndex = orderedSlugs.indexOf(over.id as MontanaDeNieblaSlug)
  ...
  function editorFor(slug: MontanaDeNieblaSlug) {
    switch (slug) {
      case 'hero':
        return (
          <div className="flex flex-col gap-4">
            ...
            />
            <StringListEditor
              label="Símbolos (emoji o texto, uno por línea)"
              values={
                Array.isArray(cfg(slug).simbolos)
                  ? (cfg(slug).simbolos as unknown[]).filter((s): s is string => typeof s === 'string')
                  : []
              }
              onChange={(next) => setField(slug, 'simbolos', next)}
            />
      ...
      case 'narrativa': {
        ...
        const norm = bloques.map((b) => ({
          edicion: typeof b.edicion === 'string' ? b.edicion : '',
        ...
        return (
          <div className="flex flex-col gap-3">
            <Field label="Eyebrow">
              <input
                className={inputCls}
                placeholder="LA HISTORIA CONTINÚA"
            ...
            <button
              ...
              onClick={() => setBloques([...norm, { edicion: '', texto: '' }])}
            >
              + Agregar bloque
            </button>
            {norm.map((b, i) => (
              <div key={i} className="border border-[#EEEEEE] bg-white p-3">
                ...
                <Field label="Edición">
                  <input className={inputCls} value={b.edicion} onChange={(e) => {
                    const n = [...norm]; n[i] = { ...n[i], edicion: e.target.value }; setBloques(n)
                  }} />
                </Field>
                <Field label="Texto">
      ...
      case 'facciones': {
        const fc = cfg(slug)
        const redSun = { ...emptyFaccion(), ...(typeof fc.red_sun === 'object' && fc.red_sun ? fc.red_sun : {}) }
        const lux = {
          ...emptyFaccion(),
          ...(typeof fc.lux_et_umbra === 'object' && fc.lux_et_umbra ? fc.lux_et_umbra : {}),
        }
        const setFaction = (side: 'red_sun' | 'lux_et_umbra', next: ReturnType<typeof emptyFaccion>) => {
          patch(slug, { [side]: next })
        }
        const codigoEditor = (
          side: 'red_sun' | 'lux_et_umbra',
          data: ReturnType<typeof emptyFaccion>
        ) => {
          const codigo = data.codigo_uniforme ?? emptyFaccion().codigo_uniforme
          const setCodigo = (partial: Partial<typeof codigo>) => {
            setFaction(side, {
              ...data,
              codigo_uniforme: { ...codigo, ...partial },
            })
          }
          return (
            <div className="mt-3 flex flex-col gap-3 border-t border-[#EEEEEE] pt-3">
              <Field label="Código — título">
                <input
                  className={inputCls}
                  value={codigo.titulo}
                  onChange={(e) => setCodigo({ titulo: e.target.value })}
                />
              </Field>
              <StringListEditor
                label="Permitidos"
                values={codigo.permitidos ?? []}
                onChange={(permitidos) => setCodigo({ permitidos })}
              />
              <StringListEditor
                label="Prohibidos"
                values={codigo.prohibidos ?? []}
                onChange={(prohibidos) => setCodigo({ prohibidos })}
              />
            </div>
          )
        }
            ...
            <div className="mt-3 flex flex-col gap-3">
              <Field label="Nombre">
                <input
                  className={inputCls}
                  value={data.nombre}
                  onChange={(e) => setFaction(side, { ...data, nombre: e.target.value })}
                />
              </Field>
              <Field label="Descripción">
                <textarea
                  ...
                  onChange={(e) => setFaction(side, { ...data, descripcion: e.target.value })}
                />
              </Field>
              <Field label="Imagen operadores">
                <ImageUploadInput
                  slug={imgSlug}
                  value={data.imagen_url}
                  onChange={(u) => setFaction(side, { ...data, imagen_url: u })}
                />
              </Field>
              <Field label="Logo / sello">
                <ImageUploadInput
                  slug={`${imgSlug}-logo`}
                  value={data.logo_url}
                  onChange={(u) => setFaction(side, { ...data, logo_url: u })}
                />
              </Field>
              {codigoEditor(side, data)}
              <Field label="Contacto nombre">
                <input
                  className={inputCls}
                  value={data.contacto_nombre}
                  onChange={(e) => setFaction(side, { ...data, contacto_nombre: e.target.value })}
                />
              </Field>
              <Field label="WhatsApp / URL">
                <input
                  className={inputCls}
                  value={data.contacto_whatsapp}
                  onChange={(e) => setFaction(side, { ...data, contacto_whatsapp: e.target.value })}
                />
              </Field>
            </div>
          </details>
        ...
        return (
          <div className="flex flex-col gap-4">
            ...
            {factionPanel('red_sun', 'Red Sun', 'facciones-red-sun', redSun)}
            {factionPanel('lux_et_umbra', 'Lux et Umbra', 'facciones-lux', lux)}
            <Field label="Nota">
              <textarea
                rows={3}
                className={inputCls}
                value={str(slug, 'nota')}
                onChange={(e) => setField(slug, 'nota', e.target.value)}
              />
            </Field>
          </div>
        )
      ...
      case 'puntos_victoria': {
        const criterios = (Array.isArray(cfg(slug).criterios) ? cfg(slug).criterios : []) as {
          tipo: string
          texto: string
        }[]
        const norm = criterios.map((c) => ({
          tipo: c.tipo === 'resta' ? ('resta' as const) : ('suma' as const),
          texto: typeof c.texto === 'string' ? c.texto : '',
        }))
        const setCriterios = (next: typeof norm) => setField(slug, 'criterios', next)
        return (
          <div className="flex flex-col gap-4">
            <Field label="Eyebrow">
              <input
            ...
            <Field label="Descripción">
              <textarea
                rows={4}
                className={inputCls}
                value={str(slug, 'descripcion')}
                onChange={(e) => setField(slug, 'descripcion', e.target.value)}
              />
            </Field>
            <button
              type="button"
              className="self-start border border-[#DDDDDD] bg-white px-3 py-2 text-[10px]"
              style={jost}
              onClick={() => setCriterios([...norm, { tipo: 'suma', texto: '' }])}
            >
              + Criterio
            </button>
            {norm.map((c, i) => (
              <div key={i} className="border border-[#EEEEEE] bg-white p-3">
                <div className="mb-2 flex gap-2">
                  ...
                  <button
                    ...
                    style={jost}
                    onClick={() => setCriterios(norm.filter((_, j) => j !== i))}
                  >
                    Eliminar
                  </button>
                </div>
                <Field label="Tipo">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      n[i] = {
                        ...n[i],
                        tipo: e.target.value === 'resta' ? 'resta' : 'suma',
                      }
                      setCriterios(n)
                    }}
                  >
      ...
      case 'inscripcion': {
        ...
        const norm = ventanas.map((v) => ({
          ...
          precio_general: Number(v.precio_general) || 0,
          precio_preferente: Number(v.precio_preferente) || 0,
        }))
          ...
          <div className="flex flex-col gap-4">
            ...
            <Field label="Subtítulo">
              <textarea
                rows={2}
                className={inputCls}
                value={str(slug, 'subtitulo')}
                onChange={(e) => setField(slug, 'subtitulo', e.target.value)}
              />
            </Field>
            <button
              type="button"
              ...
              <div key={i} className="border border-[#EEEEEE] bg-white p-3">
                <div className="mb-2 flex gap-2">
                  ...
                  <button
                    ...
                    className="text-[10px] text-[#CC4B37]"
                    style={jost}
                    onClick={() => setVentanas(norm.filter((_, j) => j !== i))}
                  >
                    Eliminar
                  </button>
                </div>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <Field label="Desde">
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...norm]
                        n[i] = { ...n[i], fecha_desde: e.target.value }
                        setVentanas(n)
                      }}
                    />
                  </Field>
                  <Field label="Hasta">
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...norm]
                        n[i] = { ...n[i], fecha_hasta: e.target.value }
                        setVentanas(n)
                      }}
                    />
                  </Field>
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <Field label="Precio general">
                    <input
                      type="number"
                      className={inputCls}
                      value={v.precio_general}
                      onChange={(e) => {
                        const n = [...norm]
                        n[i] = { ...n[i], precio_general: Number(e.target.value) }
                        setVentanas(n)
                      }}
                    />
                  </Field>
                  <Field label="Precio preferente">
                    <input
                      type="number"
                      className={inputCls}
                      value={v.precio_preferente}
                      onChange={(e) => {
                        const n = [...norm]
                        n[i] = { ...n[i], precio_preferente: Number(e.target.value) }
                        setVentanas(n)
                      }}
                    />
                  </Field>
            ...
            <Field label="Nota preferente">
              <textarea
                rows={2}
                className={inputCls}
                value={str(slug, 'nota_preferente')}
                onChange={(e) => setField(slug, 'nota_preferente', e.target.value)}
              />
            </Field>
            <Field label="Nota cambio de nombre">
              <textarea
                rows={2}
                className={inputCls}
                value={str(slug, 'nota_cambio_nombre')}
                onChange={(e) => setField(slug, 'nota_cambio_nombre', e.target.value)}
              />
            </Field>
            <Field label="CTA 1 texto">
              <input
                className={inputCls}
                value={str(slug, 'cta1_texto')}
                onChange={(e) => setField(slug, 'cta1_texto', e.target.value)}
              />
            </Field>
            <Field label="CTA 1 link">
              <input
                className={inputCls}
                value={str(slug, 'cta1_link')}
                onChange={(e) => setField(slug, 'cta1_link', e.target.value)}
              />
            </Field>
            <Field label="CTA 2 texto">
              <input
                className={inputCls}
                value={str(slug, 'cta2_texto')}
                onChange={(e) => setField(slug, 'cta2_texto', e.target.value)}
              />
            </Field>
            <Field label="CTA 2 link">
              <input
                className={inputCls}
                value={str(slug, 'cta2_link')}
                onChange={(e) => setField(slug, 'cta2_link', e.target.value)}
              />
            </Field>
          </div>
        )
        ...
        return (
          <div className="flex flex-col gap-4">
            ...
            <Field label="Pie de página">
              <input className={inputCls} value={str(slug, 'pie_pagina')} onChange={(e) => setField(slug, 'pie_pagina', e.target.value)} />
            </Field>
    ...
    <div className="flex flex-col gap-0" style={lato}>
      <div className="mb-5 border-b border-[#EEEEEE] pb-4">
        <h2 className="text-[13px] tracking-[0.14em] text-[#111111]" style={jost}>
          Editor de landing — Montaña de Niebla VII
        </h2>
        <p className="mt-1 text-[12px] text-[#999999]" style={lato}>
          Cambios visibles en{' '}
          <a href="/montanadeniebla" target="_blank" className="text-[#CC4B37] underline">
            /montanadeniebla
          </a>
        </p>
      </div>
                      ...
                      {isOpen && (
                        <div className="border-t border-[#EEEEEE] bg-[#FAFAFA] p-5">
                          ...
                          <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-[#EEEEEE] pt-4">
                            ...
                            <a href="/montanadeniebla" target="_blank" className="text-[11px] text-[#CC4B37] hover:underline" style={jost}>Ver landing →</a>
                          </div>
                        </div>
                      )}
                    </div>
