...
import { transferFieldAction } from './transfer-actions'
...
export function TransferModal({
  ...
  const handleConfirm = async () => {
    ...
    setConfirming(true)
    setError('')
    const res = await transferFieldAction(fieldId, selectedUser.id)
    setConfirming(false)
    if ('error' in res) {
      setError(res.error)
      return
  ...
  const nameDisplay = resourceName.trim() || 'CAMPO'
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="transfer-field-modal-title"
    >
...
export function TransferFieldTrigger({
  fieldId,
  resourceName,
}: {
  fieldId: string
