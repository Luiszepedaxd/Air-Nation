...
import { TacticalGamesAdminClient } from './TacticalGamesAdminClient'
import type { TGRecord } from './TacticalGamesAdminClient'
...
export default async function AdminTacticalGamesPage() {
  ...
  const { data, error } = await db
    ...
    .in('slug', TG_SLUGS as unknown as string[])
    .order('orden', { ascending: true })

  if (error) {
    console.error('[admin/tacticalgames] blocks:', error.message)
  }
  ...
  const allowedSlugs = new Set(TG_SLUGS as readonly string[])
  const orderedRows = rows.filter((r) => allowedSlugs.has(r.slug))

  function rowToRecord(r: (typeof orderedRows)[number]): TGRecord {
    const slug = r.slug as TacticalGamesSlug
    const cfg =
  ...
  const initialBlocks: TGRecord[] = orderedRows.map(rowToRecord)
  const seen = new Set(initialBlocks.map((b) => b.slug))
  let nextOrden = initialBlocks.length + 1
  for (const slug of TG_SLUGS) {
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-solid border-[#EEEEEE] pb-4">
        <h1
          ...
          style={headingStyle}
        >
          AIRSOFT TACTICAL GAMES — LANDING
        </h1>
        <Link
          href="/tacticalgames"
          target="_blank"
          className="bg-[#111111] px-4 py-2.5 text-[10px] tracking-[0.12em] text-white transition-colors hover:bg-[#CC4B37]"
          style={{ ...headingStyle, borderRadius: 2 }}
        >
          Ver landing →
        </Link>
      </div>

      <TacticalGamesAdminClient initialBlocks={initialBlocks} />
    </div>
  )
}
