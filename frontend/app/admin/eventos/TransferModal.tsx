...
import { transferEventAction } from './transfer-actions'
...
export function TransferModal({
  ...
  const handleConfirm = async () => {
    ...
    setConfirming(true)
    setError('')
    const res = await transferEventAction(eventId, selectedUser.id)
    setConfirming(false)
    if ('error' in res) {
      setError(res.error)
      return
  ...
  const nameDisplay = resourceName.trim() || 'EVENTO'
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="transfer-event-modal-title"
    >
...
export function TransferEventTrigger({
  eventId,
  resourceName,
}: {
  eventId: string
  ...
  return (
    <>
      <button
        ...
        className="inline-block w-full border border-[#111111] px-3 py-1.5 text-center font-bold text-[0.7rem] uppercase tracking-[0.15em] text-[#111111] transition-colors hover:bg-[#111111] hover:text-white"
        style={jostHeading}
      >
        TRANSFERIR
      </button>
